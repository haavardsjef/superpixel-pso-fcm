...
@Log4j2
public class NoisyBandExperimentV2 implements IExperiment {
	@Override
	public void runExperiment() throws IOException {
		...
		String filterString = "attributes.status = 'FINISHED' and params.dataset = 'Salinas' and params.corrected = 'True'";
		...
		for (Service.Run run : runs) {
			...
			for (Service.Param param : run.getData().getParamsList()) {
				if (param.getKey().equals("clusterCenters")) {
			...
			Dataset ds = new Dataset(DatasetName.Salinas, true);
			...
			ClusterRepresentatives.RepresentativeMethod[] representativeMethods = new ClusterRepresentatives.RepresentativeMethod[]{ClusterRepresentatives.RepresentativeMethod.highestEntropy};

			MLFlow mlFlow = new MLFlow("http://35.185.118.215:8080/");

			// Initialize experiment
			mlFlow.initializeExperiment("NBEV2");
	...
	public static void main(String[] args) throws IOException {
		NoisyBandExperimentV2 clusterRepresentativeExperimentV2 = new NoisyBandExperimentV2();
